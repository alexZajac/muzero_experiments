...
As always, we begin by a standard comparison of MuZero when naïve, and when trained. On the left, we can clearly see that when not trained, the agent has no clue of what's going on.
Meanwhile, on the right, we loaded the weights from a [pretrained model](https://github.com/alexZajac/muzero_experiments/blob/master/muzero/good_results/connect4-2021-01-18--07-07-21/model.checkpoint), and obtained astonishing performance. Muzero is starting with the circle symbol, and we can clearly see that it picks the optimal move first (center of the board), builds strategy by controlling the center with diagonals, and keeps on being aggressive, forcing us to always counter his moves until we are no longer in a position to hold on. We do not consider ourselves pros of the connect4 game, but throughout the games we have played with it, it showed extraordinary performance from just a few hours of training, starting from literally, zero.
...
Once again, we familiarized ourselves with the rules, and destroyed the untrained random MuZero agent with 2 moves. But it was a whole different story with the trained agent. While we had no knowledge about this game before the project, and it was the same case with MuZero, we expected the game to be quite long. It wasn't the case on the right, where we can clearly see MuZero attacking our side, or forcing us to withdraw piece to the back of the board. We couldn't even really try an attack except taking the chick in the beginning of the game. We can clearly see in the GIF that the more the game goes on, the more we hesitate, until we are placed in a checkmate by the two elephants. It is really interesting here that MuZero learned to alternate between playing with the pieces on the board, and with the pieces from the stock, and it is that variation that led him to victory.

![Naïve Agent](./gifs/shogi_no_trained_vs_h.gif)

![Trained Agent](./gifs/shogi_trained_vs_h.gif)

We also had access to the tensorboard results of the training phase for Animal Shogi. The dark red line is the first 100k training steps and the pink line is the next 100k training steps. We can see that the rewards follo a coherant pattern for the first 100k steps, and then stagnate. It is not much different for the losses, except for the value loss, whihc only decreases after 100k iterations: this might mean that the value network is harder to modelize, and the long term planning of sotcking/playing pieces might be related to the value evaluation function. The parameters used for training as well as the hardware setup is [available here](https://github.com/alexZajac/muzero_experiments/tree/master/muzero/good_results/animal_shogi-2021-01-24--19-21-10).
...
In this part, we wanted to compare MuZero with a more standard method adapted for board games: the Minimax Algorithm with alpha-beta pruning, referenced above.
...
Here are the 3 games where Minimax started to play first:
...
And here are the 3 games where MuZero started to play first:
...
We can see side by side 2 games, respectively when AlphaZero is starting, and then when MuZero is starting:
...
- The outcome is the same in both cases, the AlphaZero Agent wins. This is interesting since the advantaged player is the one starting, and MuZero didn't manage to win even when starting. Nevertheless, both games are pretty tight and it's only the last pieces that AlphaZero wins.
- One possible explanations to this is that the MuZero algorithm doesn't have any prior on the game rules, so it might take longer to train it in order to achieve the same performance as AlphaZero. Even in our paper we can see that MuZero exceeds AlphaZero for the game of go (the most similar to connect4) after 500k steps, and our algorithm was only trained for 100k of them, and we can see that it learns a lot after this threshold in the paper.
- Another reasons might come from the disparities in the 2 code implementations, and with the hyperparamters that both used, here are the hyperparameters for AlphaZero. With a more coherent base for the 2 algorithms and more training steps, it is almost sure that MuZero would reproduce the results in the paper, and win against his father, AlphaZero.

We also tried to increase the number of simulations for MuZero to 500, versus 50 for AlphaZero, but the latter still managed to win after a differently-looking game compared to when both agents were allowed 50 simulation steps. This reinforces the arguments stated above about the training of MuZero.
