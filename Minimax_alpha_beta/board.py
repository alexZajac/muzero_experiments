        ...
        if dir == 'vertical':
            ...
            if slope == 'positive' or slope == 'both':
                for diagIndex in range(positiveCount):
                    board[row+diagIndex][col+diagIndex] = board[row +
                                                                diagIndex][col+diagIndex].upper()
    ...
    for rowIndex in range(BOARD_HEIGHT):
        for colIndex in range(BOARD_WIDTH):
            if board[rowIndex][colIndex] != ' ':
                ...
                fourInARow, horizontalCount = horizontalCheck(
                    rowIndex, colIndex)
                ...
                fourInARow, slope, positiveCount, negativeCount = diagonalCheck(
                    rowIndex, colIndex)
                if fourInARow:
...
def printBoard(Board):
    ...
    print('')
    print(YELLOW + '         ROUND #' + str(emptyLocations) +
          WHITE, end=" ")  # print round number
    print('')
    print('')
    print("\t      1   2   3   4   5   6   7 ")
    print("\t      -   -   -   -   -   -   - ")
    ...
    for i in range(0, BOARD_HEIGHT, 1):
        ...
        for j in range(BOARD_WIDTH):
            if str(Board[i][j]) == 'x':
                print("| " + BLUE + 'o' + WHITE, end=" ")  # print colored 'x'
            elif str(Board[i][j]) == 'o':
                print("| " + RED + 'x' + WHITE, end=" ")  # print colored 'o'
            elif str(Board[i][j]) == 'X':
                print("| " + BLUE_BG + 'O' + WHITE,
                      end=" ")  # print colored 'X'
            elif str(Board[i][j]) == 'O':
                print("| " + RED_BG + 'X' + WHITE,
                      end=" ")  # print colored 'O'
            else:
